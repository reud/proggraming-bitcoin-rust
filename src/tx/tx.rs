...
impl Tx {
    ...
    pub fn parse(testnet: bool,serialization: Vec<u8>) -> Tx {
        let mut c = Cursor::new(serialization);
        let mut version = [0u8;4];
        c.read(&mut version);
        let version = u32::from_le_bytes(version);
        let tx_ins_len = read_varint(&mut c);
        let mut tx_ins = vec![];

        for i in 0..tx_ins_len {
            tx_ins.push(TxIn::parse(&mut c));
        }
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f,"tx: {} \n version: {}\n tx_ins: \n{}\n tx_outs: {}\n locktime: {}\n testnet: {}"
               ...
               ,self.clone().tx_ins.into_iter()
                   .map(|x| format!("  {}:{}", x.prev_transaction_id, x.prev_transaction_index))
                   .collect::<Vec<_>>()
                   .join("\n")
               ,u8vec_to_str(self.clone().tx_outs)
               ,u8vec_to_str(self.clone().lock_time)
               ,self.testnet)
    }
}
...
mod tests {
    ...
    fn test_encode_base58() {
        {
            let tx = Tx::parse(false,vec![1u8,0u8,0u8,0u8,0u8]);
